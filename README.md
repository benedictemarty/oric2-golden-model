...
> **This codebase was entirely generated by an AI model (Claude Opus 4.6, Anthropic)**
> under the direction and supervision of a human operator.

### Warnings

- **No formal verification**: the code has not been audited by a professional
  software engineer. While 216 unit tests pass, test coverage is not exhaustive
  and edge cases may exist.
- **Not production-grade**: this is an experimental/educational project. It should
  not be used in safety-critical, security-sensitive, or production environments
  without thorough independent review.
- **Potential inaccuracies**: hardware emulation accuracy is based on available
  documentation and reference implementations (Oricutron, EUPHORIC). Some behaviors
  may differ from real ORIC hardware.
- **Security**: the code has not undergone a security audit. Network features
  (cast server, CASTV2) should be used on trusted networks only.
- **AI limitations**: AI-generated code may contain subtle logic errors, non-idiomatic
  patterns, or architectural choices that a human developer would approach differently.
- **Maintenance**: future updates depend on AI model availability and may introduce
  regressions or inconsistencies across sessions.

Use at your own risk. Contributions and code reviews are welcome.

## Credits

- **Claude Opus 4.6 (Anthropic)** — AI code generation (architecture, implementation, tests, documentation)
- **bmarty** — Project direction, supervision, testing on real hardware
