 ...
 */
/* ═══════════════════════════════════════════════════════════════════════
 *  ULA2: render with per-cell Color RAM (no serial attributes)
 *
 *  Color RAM at $B000-$B45F: 1120 bytes, one per character cell.
 *  Each byte: ink (bits 0-2), paper (bits 3-5), alt charset (bit 6),
 *  inverse (bit 7).
 *
 *  In ULA2 mode, ALL bytes in video RAM are character/pixel data.
 *  No bytes are consumed as serial attributes. The (byte & 0x60)==0
 *  check is NOT performed — those values render as normal characters.
 * ═══════════════════════════════════════════════════════════════════════ */

static void render_ula2_text(video_t* vid, const uint8_t* memory) {
    for (int row = 0; row < 28; row++) {
        for (int col = 0; col < 40; col++) {
            uint8_t byte = memory[0xBB80 + row * 40 + col];
            uint8_t cattr = memory[ORIC_COLOR_RAM_ADDR + row * 40 + col];

            uint8_t ink   = cattr & ULA2_INK_MASK;
            uint8_t paper = (cattr & ULA2_PAPER_MASK) >> ULA2_PAPER_SHIFT;
            bool inverse  = (cattr & ULA2_INVERSE) != 0;
            /* bit 6 (ULA2_ALT_CHAR) reserved for future alternate charset */

            int sy = (row < 25) ? row * 8 : 200 + (row - 25) * 8;
            ...
            for (int cy = 0; cy < 8; cy++) {
                uint8_t bits = get_charset_byte(vid, memory, byte & 0x7F, cy);
                bool char_inv = (byte & 0x80) != 0;
                for (int bx = 5; bx >= 0; bx--) {
                    bool on = (bits & (1 << bx)) != 0;
                    if (char_inv) on = !on;
...
static void render_ula2_hires(video_t* vid, const uint8_t* memory) {
    ...
    for (int y = 0; y < 200; y++) {
        ...
        for (int col = 0; col < 40; col++) {
            ...
            uint8_t cattr = memory[ORIC_COLOR_RAM_ADDR + cell_row * 40 + col];

            uint8_t ink   = cattr & ULA2_INK_MASK;
            uint8_t paper = (cattr & ULA2_PAPER_MASK) >> ULA2_PAPER_SHIFT;

            /* All bytes are pixel data — no attribute detection */
